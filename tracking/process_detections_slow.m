function [hbboxdata_od,status,startframe] = process_detections_slow(rawbbdata,classid)
...
for i = 1:size(rawbbdata,1)
    reqinddata(i) = length(find(rawbbdata(i,1:5:end)==classid));
end
...
if maxdetclass>0
    ...
    for i = 1:size(rawbbdata,1)
        if reqinddata(i)>0
            ind = find(rawbbdata(i,1:5:end) == classid);
            for j = 1:reqinddata(i)
                bbox_od(i,(j-1)*4+1:j*4) = rawbbdata(i,(ind(j)-1)*5+2:ind(j)*5);
            end
        end
    end
