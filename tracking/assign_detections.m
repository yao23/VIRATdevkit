...
for class = 1:length(datamatrix)
    ...
    areainteg =  rectint(reshape(datamatrix{class}(framenum-fb,:)',4,...
        length(datamatrix{class}(framenum-fb,:))/4)',reshape(detdata{class}(framenum,:),4,length(detdata{class}(framenum,:))/4)');
    % Picking out the third and fourth element of datamatrix and same for
    % detections to get pascal ratio
    areamat = repmat((datamatrix{class}(framenum-fb,3:4:end).*datamatrix{class}(framenum-fb,4:4:end))',1,size(areainteg,2))+...
        repmat(detdata{class}(framenum,3:4:end).*detdata{class}(framenum,4:4:end),size(areainteg,1),1);
    ...
    for i = 1:size(match,1)
        if match(i,2) > minareathresh
            ...
            datamatrix{class}(framenum,1+4*(i-1):4*i) = detdata{class}(framenum,1+(match(i,1)-1)*4:match(i,1)*4);
            % Last detection
            statematrix{class}(framenum,i) = 4;
            % Current detection has been assigned
            currentdetect(1+(match(i,1)-1)*4:match(i,1)*4) = zeros(1,4);
        end
    end
    ...
    if length(find(currentdetect)) >=2
        ...
        currentdetectind = find(currentdetect);
